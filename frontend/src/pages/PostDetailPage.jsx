...
function PostDetailPage() {
  ...
  return (
    <main className="max-w-4xl mx-auto px-4 py-10">
      {/* Hero */}
      <header className="bg-white rounded-lg overflow-hidden mb-8 shadow-md border">
        {post.image ? (
          <div
          ...
          <div className="w-full h-48 bg-gradient-to-r from-lilac-50 to-white flex items-center justify-center">
            <div className="text-center p-6">
              <h1 className="text-3xl font-extrabold text-lilac-800 mb-2">{post.title}</h1>
              {post.category && (
                <span className="inline-block text-sm text-lilac-600 bg-lilac-50 px-3 py-1 rounded-full">
                  {post.category.name}
                </span>
              )}
            </div>
        ...
        <div className="p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-3">
              <Link to="/" className="text-sm text-lilac-600 hover:underline">Back</Link>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {related.map((r) => (
              <Link key={r._id} to={`/posts/${r._id}`} className="card p-4 hover:shadow-lg transition">
                <h4 className="text-sm font-semibold text-lilac-700 mb-2">{r.title}</h4>
